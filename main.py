...
def check_ma2_148_cross(candles, live_price, atr_value=None):
    """
    MA(2) / MA(148) touch-cross, evaluated on CLOSED candles only.
    ...
    fetched live tick. (An earlier version mixed live_price into the
    fast MA to react faster than candle close; that made the signal
    depend on the live feed and the historical feed agreeing in
    absolute terms, which is not something you can rely on across data
    providers — it produced MA148 readings far from the broker's own
    chart. This version trades a little reaction speed for actually
    being correct.)

    BUY:  fast MA was at/below slow MA, now above it (crossed up).
    SELL: fast MA was at/above slow MA, now below it (crossed down).
    """
    if len(candles) < MA148_PERIOD + 2:
        return None, None
    ...
    if fast_prev <= slow_prev and fast_now > slow_now:
        return "long", f"MA2/148 Touch Up @ {entry_price:.2f} (MA148 {slow_now:.2f})"

    if fast_prev >= slow_prev and fast_now < slow_now:
        return "short", f"MA2/148 Touch Down @ {entry_price:.2f} (MA148 {slow_now:.2f})"
