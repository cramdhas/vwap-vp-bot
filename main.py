...
# MA2/148 touch-cross strategy (fast MA reacts to the live tick, so it
...
def check_ma2_148_cross(candles, live_price, atr_value=None):
    """
    MA(2) / MA(148) touch-cross, evaluated on the LIVE price.

    Unlike the other strategies, this does not wait for the current M5
    candle to close: it recomputes the fast MA(2) using the live tick
    price in place of the still-forming candle's close, so it can fire
    the instant the fast line touches/crosses the slow MA(148) line —
    matching "enter the moment red touches blue" rather than waiting
    for bar close.

    BUY:  fast MA was at/below slow MA, live tick pushes fast MA above it.
    SELL: fast MA was at/above slow MA, live tick pushes fast MA below it.
    """
    if len(candles) < MA148_PERIOD + 2 or live_price is None:
        return None, None

    closes = [c["close"] for c in candles]

    # Slow MA(148) from completed candles — one live tick barely moves a
    # 148-period average, so it's computed on closed bars only.
    slow_now = sum(closes[-MA148_PERIOD:]) / MA148_PERIOD

    # "Previous" state: last fully closed bar, before the live tick.
    fast_prev = sum(closes[-MA2_PERIOD:]) / MA2_PERIOD
    slow_prev = slow_now

    # "Now": swap the still-forming bar's price for the live tick.
    fast_now = (closes[-1] + live_price) / 2

    # Sanity guard: a legitimate MA148 shouldn't sit absurdly far from the
    # live price. A bad data point slipping through can still skew a
    # 148-bar average badly, so this is a second line of defense on top
    # of sanitize_candles() at the data layer.
    sane_atr = atr_value if atr_value and atr_value > 0 else 5.0
    max_reasonable_gap = max(30.0, sane_atr * 12)
    if abs(slow_now - live_price) > max_reasonable_gap:
        print(
            f"[STRATEGY WARNING] MA2/148 Cross: MA148 ({slow_now:.2f}) is "
            f"{abs(slow_now - live_price):.1f} pts from live price "
            f"({live_price:.2f}) — beyond sanity threshold "
            f"({max_reasonable_gap:.1f}), skipping. Likely bad data upstream."
        )
        return None, None

    if fast_prev <= slow_prev and fast_now > slow_now:
        return "long", f"MA2/148 Touch Up @ {live_price:.2f} (MA148 {slow_now:.2f})"

    if fast_prev >= slow_prev and fast_now < slow_now:
        return "short", f"MA2/148 Touch Down @ {live_price:.2f} (MA148 {slow_now:.2f})"
...
def main():
    ...
    # NEW: MA2/148 live touch-cross strategy
    ...
    # disagree with the broker's own MA148 by 30+ points, which would
    # generate false signals, so skip it on that fallback.
    if data_source in ("xaus_chart", "xaus_intraday"):
        d, info = run_strategy(
            "MA2/148 Cross", check_ma2_148_cross, candles, price,
            current_atr if current_atr else fallback_atr,
        )
        if d:
            raw.append(("MA2/148 Cross", d, info or ""))
    else:
        print(
            f"[STRATEGY] MA2/148 Cross: skipped — data source is "
