...
def get_klines(limit=LOOKBACK_BARS):
    try:
        ...
        print(f"[DATA] XAUUSD M5 via XAUS intraday fallback: {len(candles)} completed candles")
        return candles
    ...
    for i in range(len(candles) - 3, len(candles) - OB_LOOKBACK - 1, -1):
        ...
        if c["close"] < c["open"]:
            impulse = any(
                ...
                and abs(candles[j]["close"] - candles[j]["open"]) > avg_body * OB_IMPULSE_MULT
...
def main():
    ...
    now_ts = time.time()
    state["zone_last_fired"] = {
        k: v for k, v in state.get("zone_last_fired", {}).items() if now_ts - v < ZONE_DEDUP_SECONDS
    }
    save_state(state)
